...
def kmeans_plus_plus(norms: np.ndarray, num_samples: int):
    probs = norms / np.sum(norms)
    inds = np.random.choice(np.arange(norms.shape[0]), num_samples, replace=False, p=probs)
    return inds
