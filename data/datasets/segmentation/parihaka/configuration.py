...
def get_parihaka_test_config(loaders: SeismicLoaderObject, cfg: BaseConfig, uspec_analysis: bool = False):
    ...
    if cfg.data.seismic.sets.training_xline:
        set_shape = (loaders.data_config.train_xline_len, 255, 590)
        if uspec_analysis:
    ...
    if cfg.data.seismic.sets.training_inline:
        set_shape = (loaders.data_config.train_inline_len, 255, 391)
        if uspec_analysis:
            tracker = USPECTracker(set_shape, cfg.uspec_configs.num_seeds)
        else:
            tracker = SegmentationTracker(set_shape)
    ...
    if cfg.data.seismic.sets.test1_inline:
        set_shape = (loaders.data_config.test1_inline_len, 255, 590)
        if uspec_analysis:
            tracker = USPECTracker(set_shape, cfg.uspec_configs.num_seeds)
        else:
            tracker = SegmentationTracker(set_shape)
        stats = StatObject(loaders.test1_inline, tracker, 'test_inline')
        out.append(stats)
    if cfg.data.seismic.sets.test1_xline:
        set_shape = (loaders.data_config.test1_xline_len, 255, 391)
        if uspec_analysis:
            tracker = USPECTracker(set_shape, cfg.uspec_configs.num_seeds)
        else:
            tracker = SegmentationTracker(set_shape)
        stats = StatObject(loaders.test1_xline, tracker, 'test_xline')
        out.append(stats)
...
def get_parihaka(cfg: BaseConfig, idxs: np.ndarray, is_train: bool = True):
    ...
    test_inlines_samples, test_inlines_labels = get_unique_sections(test_path, prefix='inline')
    test_crosslines_samples, test_crosslines_labels = get_unique_sections(test_path, prefix='crossline')
    ...
    data_configs.test1_inline_set = test_inlines_samples
    data_configs.test1_inline_labels = test_inlines_labels
    data_configs.test1_inline_len = len(test_inlines_samples)

    data_configs.test1_xline_set = test_crosslines_samples
    data_configs.test1_xline_labels = test_crosslines_labels
    data_configs.test1_xline_len = len(test_crosslines_samples)
